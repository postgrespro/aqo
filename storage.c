			 ...
			 bool learn_aqo, bool use_aqo, bool auto_tuning)
{
			 ...
			 !TransactionIdIsValid(snap.xmax))
	{
		...
		else
		{
			 ...
			 */
			elog(ERROR, "AQO feature space data for signature (%lu, %lu) concurrently"
		 ...
		 List **relids)
{
	...
	ScanKeyInit(&key[0], 1, BTEqualStrategyNumber, F_INT4EQ, Int64GetDatum(fhash));
	...
	else if (find_ok)
	{
		...
		if (DatumGetInt32(values[2]) == ncols)
		{
			...
			elog(ERROR, "unexpected number of features for hash (%lu, %d):\
		   ...
		   double **matrix, double *targets, List *relids)
{
	...
	else if (!TransactionIdIsValid(snap.xmin) && !TransactionIdIsValid(snap.xmax))
	{
		...
		else
		{
			 ...
			 * only in the case of changes made by third-party code.
			 */
			elog(ERROR, "AQO data piece (%lu %d) concurrently updated"
...
update_aqo_stat(uint64 qhash, QueryStat *stat)
{
	...
	else if (!TransactionIdIsValid(snap.xmin) && !TransactionIdIsValid(snap.xmax))
	{
		...
		else
		{
			 ...
			 * only in the case of changes made by third-party code.
			 */
			elog(ERROR, "AQO statistic data for query signature %lu concurrently"
						" updated by a stranger backend.",
				 qhash);
		}
	}
